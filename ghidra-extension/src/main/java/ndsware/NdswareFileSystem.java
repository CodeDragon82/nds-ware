...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = NdswareFileSystem.MyFileSystemFactory.class)
public class NdswareFileSystem implements GFileSystem {
	 ...
	 */
	public NdswareFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + NdswareFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<NdswareFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public NdswareFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			NdswareFileSystem fs = new NdswareFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
